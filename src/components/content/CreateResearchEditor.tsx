...
export default function CreateResearchEditor({
  ...
  const [selectedHeading, setSelectedHeading] = useState("Normal"); // Track selected option
...
// ===== REFERENCE STATE (NEW) =====
  ...
  // ===== REFERENCE HELPERS (NEW) =====
  const addReference = (text: string) => {
    const trimmed = text.trim();
    if (!trimmed) {
      setReferenceError("Reference text cannot be empty");
      return;
    }
    if (selectedReferences.includes(trimmed)) {
      setReferenceError("Reference already added");
      return;
    }
    const updatedReferences = [...selectedReferences, trimmed];
  ...
  const removeReference = (index: number) => {
    ...
    setSelectedReferences(updatedReferences);
    if (onReferencesChange) onReferencesChange(updatedReferences); // Pass up to parent
  };

    const handleReferenceSubmit = (e?: React.KeyboardEvent) => { // FIXED: Only KeyboardEvent now (no FormEvent)
    if (e && e.key !== "Enter") return; // Enter key only
    addReference(referenceInput); // Direct add, no preventDefault needed
  };


  // Open popup: Clear input/error
