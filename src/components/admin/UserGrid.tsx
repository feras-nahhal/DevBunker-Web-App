...
export default function UserGrid() {
  ...
  const [itemsPerPage, setItemsPerPage] = useState<number>(() => {
    ...
    useEffect(() => {
      setCurrentPage(1);
    }, [itemsPerPage, search, selectedStatuses, createdFrom, createdTo, selectedRoles]);
...
if (createdFrom || createdTo) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Close statuses dropdown if clicked outside
      ...
      >
        ...
        <div className="w-full border-b border-[rgba(145,158,171,0.2)] bg-white/[0.05] px-5 py-4">
          ...
          <div className="flex flex-col gap-4 mb-3 md:flex-row md:flex-wrap md:items-end md:gap-3">
            ...
            <div ref={rolesRef} className="w-full md:w-[220px] relative">
          ...
          {selectedRoles.map((role) => (
            ...
            >
              ...
              <button
                ...
                aria-label={`Remove status filter ${role}`}
              >
                ×
              </button>
            </div>
