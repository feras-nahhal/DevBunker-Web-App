...
const clearAllFilters = () => {
  ...
  const goToPage = (page: number) => {
    ...
    const useresRef = useRef<HTMLDivElement>(null);
  
    useEffect(() => {
      const handleClickOutside = (event: MouseEvent) => {
        // Close roles dropdown if clicked outside
        ...
        // Close categories dropdown if clicked outside
        if (useresRef.current && !useresRef.current.contains(event.target as Node)) {
          setAuthorEmailDropdownOpen(false);
        }
      };
...
<div className="w-full border-b border-[rgba(145,158,171,0.2)] bg-white/[0.05] px-5 py-4">
  ...
  {/* Mobile Collapse Button */}
    ...
    <div ref={categoriesRef} className="w-full md:w-[220px] relative">
     ...
     {/* NEW: 👤 Author Emails Dropdown */}
      <div ref={useresRef} className="w-full md:w-[220px] relative">
